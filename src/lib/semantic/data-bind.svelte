      ...
      if (watcher) {
         ...
         switch (watcher.mode) {
            ...
            case "slider":
               ...
               break;
            default:
               throw new Error(`Parent of <Data> component has unrecognized store type: ${watcher.mode}`);
         }
      } else {
         ...
         if (watcher) {
            if (watcher.mode !== "input") {
               throw new Error(`Preceding <input> of <Data> component has unrecognized store type: ${watcher.mode}`);
            }
         } else {
